...
def start_quiz_generation(files):
    ...
    # AIへの指示を最適化：記述式問題と選択式問題を正しく区別して、記述式ならoptionsを空にするように指示
    prompt = """PDFからクイズを作成してください。

【重要ルール】
1. 問題数は6問以上15問以下にすること。
2. PDF内に既存の確認テストや設問（記述式・選択式問わず）が含まれている場合は、その元々の形式（記述式か選択式か）を忠実に守って優先的に利用すること。
3. PDFの文脈から記述式問題（一問一答、用語の記述解答、論述など）がふさわしいと判断される場合は、選択肢（options）を必ず空のリスト `[]` にすること。
4. 選択式問題にする場合のみ、選択肢（options）に4つ程度の具体的な選択肢を入れること。記述式が適した問題に対して無理に選択肢を作って選択式に変更しないこと。
5. 出力は必ず以下のJSON形式にすること。

{
  "quizzes": [
    {
      "question": "問題文（記述式の場合は『〜を答えよ』、選択式の場合は『〜を以下から選べ』などの明確な文末にすること）",
      "options": [],  // 記述式なら []、選択式なら ["選択肢1", "選択肢2", "選択肢3", "選択肢4"]
      "answer": "正解の文字列",
      "explanation": "詳しい解説"
    }
  ]
