...
def summarize_text_cached(text: str) -> str:
    ...
    prompt = """あなたは学習用資料を整理するアシスタントです。
以下の資料テキストを、日本語で分かりやすく要約してください。

【出力方針】
- 内容に応じて自然な構造でまとめる（章数は固定しない）
- 見出し + 箇条書きを基本とする
- 重要な概念・制度名・原則名・条文・キーワードは **太字** にする
- スライド形式の場合は、論点ごとにまとめ直す
- 余計な前置きは禁止（「承知しました」など出さない）
